...
public abstract class CandidateRepositoryTest {
    ...
    void findByName() {
        ...
        Candidate candidate2 = Instancio.of(Candidate.class).set(field("familyName"), "das Cove").create();

        CandidateQuery candidateQuery = new CandidateQuery.Builder().name("das Cove").build();

        repository().save(List.of(candidate1, candidate2));

        List<Candidate> result = repository().find(candidateQuery);
